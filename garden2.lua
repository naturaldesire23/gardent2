...
function load2xScript()
    ...
    local placements = {
        ...
        -- Rafflesia (Mid Game) - Slot 2 (NO UPGRADES)
        {
    ...
    local function placeUnit(unitName, slot, data)
        ...
        warn("[Placing] "..unitName.." in slot "..slot)
    end

    local function upgradeRepeatedly(slot, times, delayBetween)
        for i = 1, times do
            task.delay((i * delayBetween), function()
                -- Try multiple upgrade methods to ensure it works
                pcall(function() remotes.UpgradeUnit:InvokeServer(slot) end)           -- Method 1: Slot number
                pcall(function() remotes.UpgradeUnit:InvokeServer("slot_"..slot) end)  -- Method 2: "slot_X"
                pcall(function() remotes.UpgradeUnit:InvokeServer(tostring(slot)) end) -- Method 3: String slot
                
                warn("[Upgrade Attempt] Slot "..slot.." | Attempt "..i.."/"..times)
            end)
        end
    end
    ...
    local function startGame()
        ...
        -- Upgrade only Slingshot and Golem Dragon (NO Rafflesia upgrades)
        task.delay(50, function() upgradeRepeatedly(1, 6, 6) end)  -- Slingshot: 6 attempts, 6s apart
        task.delay(90, function() upgradeRepeatedly(3, 6, 6) end)  -- Golem Dragon: 6 attempts, 6s apart
    end
...
function load3xScript()
    ...
    local function upgradeRepeatedly(slot, times, delayBetween)
        for i = 1, times do
            task.delay((i * delayBetween), function()
                -- Try multiple upgrade methods to ensure it works
                pcall(function() remotes.UpgradeUnit:InvokeServer(slot) end)
                pcall(function() remotes.UpgradeUnit:InvokeServer("slot_"..slot) end)
                pcall(function() remotes.UpgradeUnit:InvokeServer(tostring(slot)) end)
    ...
    local function startGame()
        ...
        -- Upgrade only Slingshot and Golem Dragon (NO Rafflesia upgrades)
        task.delay(40, function() upgradeRepeatedly(1, 6, 4) end)  -- Slingshot: 6 attempts, 4s apart
        task.delay(65, function() upgradeRepeatedly(3, 6, 4) end)  -- Golem Dragon: 6 attempts, 4s apart
    end
