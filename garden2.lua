...
function load2xScript()
    ...
    -- Improved placeUnit function with better ID handling
    local function placeUnit(unitName, slot, data)
        ...
        if success and result then
            -- Add a small delay to ensure server has processed the placement
            task.wait(0.5)
            unitIDs[slot] = result
            warn("[Placing] "..unitName.." | ID: "..tostring(result))
            return true
        else
            warn("[Placing Failed] "..unitName.." | Error: "..tostring(result))
            return false
        end
    end

    -- Improved upgradeUnit function with error checking
    local function upgradeUnitSafe(slot, times)
        if not unitIDs[slot] then
            warn("[Upgrade Failed] No unit ID for slot "..slot)
            return
        end
        
        -- Wait a bit longer to ensure unit is fully placed
        task.wait(3)
        
        for i = 1, times do
            task.spawn(function()
                task.wait(i * 2) -- Wait between upgrades
                
                local success, result = pcall(function()
                    return remotes.UpgradeUnit:InvokeServer(unitIDs[slot])
                end)
                
                if success then
                    warn("[Upgrading] Slot "..slot.." | ID: "..unitIDs[slot].." | Level "..i.." | Success")
                else
                    warn("[Upgrading Failed] Slot "..slot.." | ID: "..unitIDs[slot].." | Level "..i.." | Error: "..tostring(result))
                end
            end)
        end
    end
    ...
    local function startGame()
        ...
        -- Place units and track success
        for _, p in ipairs(placements) do
            task.delay(p.time, function()
                local placed = placeUnit(p.unit, p.slot, p.data)
                if placed then
                    -- Schedule upgrades only if placement succeeded
                    if p.slot == 1 then
                        task.delay(15, function() upgradeUnitSafe(1, 4) end) -- Wait longer after placement
                    elseif p.slot == 2 then
                        task.delay(10, function() upgradeUnitSafe(2, 4) end)
                    elseif p.slot == 3 then
                        task.delay(8, function() upgradeUnitSafe(3, 4) end)
                    end
                end
            end)
        end
...
function load3xScript()
    ...
    local function upgradeUnitSafe(slot, times)
        ...
        for i = 1, times do
            task.spawn(function()
                task.wait(i * 1.5) -- Faster upgrades for 3x speed
        ...
        for _, p in ipairs(placements) do
            task.delay(p.time, function()
                ...
                if placed then
                    -- Schedule upgrades only if placement succeeded
                    if p.slot == 1 then
                        task.delay(10, function() upgradeUnitSafe(1, 4) end) -- Wait after placement
                    elseif p.slot == 2 then
                        task.delay(8, function() upgradeUnitSafe(2, 4) end)
                    elseif p.slot == 3 then
                        task.delay(6, function() upgradeUnitSafe(3, 4) end)
                    end
                end
            end)
        end
